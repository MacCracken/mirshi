...
**Direction 1 (AGNOS→Linux) is feature-complete** — every defined, non-kernel-only agnos syscall is handled. v1.0 cut the hardened / audited / optimized / frozen foundation (agnos userland in Docker, no QEMU); v1.1–v1.4 added the full net band (TCP client + server, UDP, ICMP); v1.5.0 **multi-process** (`spawn`/`waitpid`); v1.6.0 **signals** (`kill`/`sigprocmask`/`signalfd`); v1.7.0 **I/O multiplexing** (`epoll`/`timerfd`/`pipe`); v1.8.0 **info getters + advisory locks** (`getuid`/`uname`/`sysinfo`/`flock`); v1.9.0 **tty sizing** (`winsize`). The only ENOSYS rows left are the agnos-kernel-only ops + the undefined ABI gaps. The authoritative version is [`VERSION`](VERSION) + [`CHANGELOG.md`](CHANGELOG.md); the live state snapshot is [`docs/development/state.md`](docs/development/state.md). The one remaining milestone is the **v2.0.0 direction-2 "swallow"** (Linux binaries on the agnos kernel) — see the [roadmap](docs/development/roadmap.md).
