...
- **Info getters + locks** — `getuid#15`/`uname#34`/`sysinfo#35`/`flock#59`: the info getters supervisor-emulated (synthesized agnos-native structs — `getuid`→0, `uname`/`sysinfo` from live host values), `flock` execute-in-child (real inode-keyed advisory lock), [ADR 0016](docs/adr/0016-info-getters-emulated-flock-executed.md).
- **tty sizing** — `winsize#60`: supervisor-emulated `(cols<<16)|rows` from the controlling terminal's `TIOCGWINSZ` (80×24 default when headless), the whole direction-1 graphics surface, [ADR 0017](docs/adr/0017-winsize-emulated-tiocgwinsz.md).
...
**Direction 1 (AGNOS→Linux)** handles the full frozen 0–61 syscall surface, and **v1.10.0 opens the exec-band re-sync** to agnos's newer userland ABI (agnos grew syscalls past 61). v1.0 cut the hardened / audited / optimized / frozen foundation (agnos userland in Docker, no QEMU); v1.1–v1.4 added the full net band (TCP client + server, UDP, ICMP); v1.5.0 **multi-process** (`spawn`/`waitpid`); v1.6.0 **signals** (`kill`/`sigprocmask`/`signalfd`); v1.7.0 **I/O multiplexing** (`epoll`/`timerfd`/`pipe`); v1.8.0 **info getters + advisory locks** (`getuid`/`uname`/`sysinfo`/`flock`); v1.9.0 **tty sizing** (`winsize`); **v1.10.0** **argv forwarding** + **`execwait#37`** (agnsh's `run` — the exec-band opener); **v1.10.1** **`spawn_path#43`** (the stdlib `exec_*` path) on **cyrius 6.4.19**; **v1.10.2** **`exec_redirect#62`** (agnsh `>` / pipes, via `pidfd_getfd`) + **`symlink#63`** — the exec-band re-sync **complete**; **v1.11.1** **`kbscan#42`** (full-screen keyboard input for agnos TUIs — cyim's per-key channel, synthesized as raw Set-1 scancodes from mirshi's stdin) + **`readlink#70`** (symlink introspection). Remaining ENOSYS: the agnos-kernel-only ops. The authoritative version is [`VERSION`](VERSION) + [`CHANGELOG.md`](CHANGELOG.md); the live state snapshot is [`docs/development/state.md`](docs/development/state.md). The one remaining milestone is the **v2.0.0 direction-2 "swallow"** (Linux binaries on the agnos kernel) — see the [roadmap](docs/development/roadmap.md).
