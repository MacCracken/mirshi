...
**Direction 1 (AGNOS→Linux) runs the net band, multi-process, signals, I/O multiplexing, and the info-getter long-tail.** v1.0 cut the hardened / audited / optimized / frozen foundation (agnos userland in Docker, no QEMU); v1.1–v1.4 added the full net band (TCP client + server, UDP, ICMP); v1.5.0 added **multi-process** (`spawn`/`waitpid`); v1.6.0 added **signals** (`kill`/`sigprocmask`/`signalfd`); v1.7.0 added **I/O multiplexing** (`epoll`/`timerfd`/`pipe`); v1.8.0 added **info getters + advisory locks** (`getuid`/`uname`/`sysinfo`/`flock`). The authoritative version is [`VERSION`](VERSION) + [`CHANGELOG.md`](CHANGELOG.md); the live state snapshot is [`docs/development/state.md`](docs/development/state.md). Next up — `winsize` tty sizing, and the direction-2 swallow — see the [roadmap](docs/development/roadmap.md).
